...
export function Dropdown({ children, label, openTimeout = 150 }: DropdownProps) {
  ...
  return (
    <div className='relative inline-block' onMouseEnter={onMouseEnter} onMouseLeave={onMouseLeave}>
      <button
        className={`font-secondary text-lg flex items-center gap-2 ${isOpen ? 'text-zinc-100 ' : 'text-zinc-400'}`}
        onClick={() => setIsOpen(!isOpen)}
      >
        {label}{' '}
        <BsTriangleFill className={`inline-block text-xs transition-all ${isOpen ? 'rotate-180' : 'rotate-0'}`} />
      </button>
...
export function DropdownLink({ label, href, className, children }: DropdownLinkProps) {
  return (
    <Link href={href} className='flex flex-row gap-2 items-center'>
      {label && <span className={`text-base text-zinc-400 hover:text-purple-500 ${className}`}>{label}</span>}
      {children}
    </Link>
  )
}
